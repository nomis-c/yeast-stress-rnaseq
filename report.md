...
Raw paired-end reads were downloaded from SRA using `fasterq-dump`
(SRA Toolkit; Leinonen et al. 2011) and quality-trimmed with `fastp`
(Chen et al. 2018) (adapter auto-detection, Q20 minimum quality,
...
Transcript-level counts were summarised to gene level via `tximport`
(Soneson et al. 2015) and imported into `DESeq2` (Love et al. 2014) for
differential expression analysis (heat vs. control). Genes with fewer than 10 reads in at least
...
GO over-representation analysis was performed with `clusterProfiler`
(Wu et al. 2021) on the Biological Process ontology (`org.Sc.sgd.db`,
ORF keytype, Benjamini-Hochberg correction). Enrichment was run separately
on the up- and down-regulated DEGs, using the set of expressed genes that
passed the low-count filter as the background universe rather than the whole
annotated genome. Splitting by direction of regulation keeps the resulting
terms interpretable, since a combined analysis is numerically dominated by
the larger repressed gene set.

The full pipeline is implemented in
Snakemake and available at https://github.com/nomis-c/yeast-stress-rnaseq.
...
(padj ≤ 0.0005). The repressed side yields 78 significant terms,
...
(padj ≤ 0.0005), giving 35 significant terms. The induced side is
...
The results confirm the hypothesis: the ESR signature is recovered in full by
a simple heat-vs-control comparison. The heat shock response is the dominant
...
The two padj thresholds (0.05 and 0.0005) produce highly similar
enrichment profiles — the top terms and their ranking are unchanged, with
only minor differences further down the list — confirming the signal is
robust and not dependent on a particular statistical cutoff.
...
4:1521. doi:10.12688/f1000research.7563.2
